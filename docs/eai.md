...
Each release has Claude.ai and first-party API files. `pipeline/download_aei.py`
downloads the registered releases into `input/`. `pipeline/clean_aei.py` filters raw
...
`pipeline/convert_onet.py` converts the O*NET 20.1 task statements workbook to
...
`pipeline/build_iwa_onet_mapping.py` reads local O*NET 30.2 Excel files in
...
`pipeline/convert_oews.py` converts local OEWS Excel files to lowercase CSVs under
`output/oews/`. The repo currently has national OEWS files for 2019 through
2024, plus the May 2024 all-data file. `pipeline/panel.py` stacks the 2021-2024
national files into `output/oews_panel.csv` with a balanced-panel indicator.

Important year distinction:

- `pipeline/occupational_characteristics.py` defaults to May 2022 OEWS.
...
`pipeline/download_eloundou.py` downloads `input/eloundou_occ_level.csv` from
the GPTs-are-GPTs repository.
`pipeline/occupational_characteristics.py` reads `input/eloundou_occ_level.csv`, collapses
...
Script: `pipeline/occupational_characteristics.py`
...
`pipeline/occupational_characteristics.py` run, so it uses May 2022 OEWS. Running the
...
Script: `pipeline/build_iwa_onet_mapping.py`
...
Script: `pipeline/build_openai_iwa_oews.py`
...
Every output directory has a `codebook.md` with variable definitions for the
files it contains. Codebooks are generated by the pipeline and analysis
scripts themselves (via `eai/codebook.py`), so they stay in sync with the
code: edit the definitions in the generating script, not in the codebook
files.

One scale caveat worth knowing: the `_pc` per-capita columns in the AEI
occupation panels are per worker, while the OpenAI wage analysis reports
`mean_share_per_million_workers` per million workers. The codebooks state
this on each affected column.

## Output And Git Hygiene

The repo keeps raw source downloads out of version control where possible.
`input/.gitignore` excludes local data drops such as the O*NET 30.2 folder,
the OpenAI Signals release folder, and downloaded AEI release files (except
