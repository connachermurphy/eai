...
OpenAI Signals IWA-level message shares allocated to SOC 2018 occupations by employment within each IWA. All shares are proportions in [0, 1].
...
| Variable | Definition |
| --- | --- |
| `openai_measure` | Which OpenAI Signals series the row comes from: us_all_messages_iwa_share (all U.S. consumer ChatGPT messages) or us_work_related_messages_iwa_share (work-related U.S. messages). |
...
One row per SOC 2018 occupation with mean apportioned OpenAI shares across all available months.
