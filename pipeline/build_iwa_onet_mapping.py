...
from eai.codebook import update_codebook  # noqa: E402
...
LINK_BASIS_DEF = (
    "Counted over the Tasks-to-DWAs rows connecting the IWA to the occupation "
    "(join path: IWA Reference -> DWA Reference -> Tasks to DWAs)."
)
TASK_TYPE_DEF = (
    "O*NET Task Type: Core (relevance >= 67% and mean importance >= 3.0) or "
    "Supplemental; blank rows are retained as Unclassified."
)
WEIGHT_COLS = [
    (
        f"occupation_weight_within_iwa_{basis}",
        f"Candidate allocation weight: the occupation's {label} divided by "
        "the sum of that basis across all occupations linked to the IWA "
        "(sums to 1 within IWA; NA when the denominator is 0).",
    )
    for basis, label in [
        ("task_count", "distinct task count"),
        ("task_dwa_links", "task-DWA link row count"),
        ("dwa_count", "distinct DWA count"),
        ("core_task_count", "distinct Core task count"),
        ("classified_task_count", "classified (Core + Supplemental) task count"),
    ]
]


def _count_cols(prefix: str = "") -> list[tuple[str, str]]:
    """Task/DWA count definitions, optionally with the link_ prefix."""
    scope = "for the IWA-occupation link" if prefix else "within the group"
    return [
        (
            f"{prefix}task_count",
            f"Distinct O*NET task IDs {scope}. {LINK_BASIS_DEF}",
        ),
        (
            f"{prefix}task_dwa_link_count",
            f"Task-to-DWA link rows {scope} (a task can map to several DWAs "
            "under one IWA).",
        ),
        (f"{prefix}dwa_count", f"Distinct DWA IDs {scope}."),
        (
            f"{prefix}core_task_count",
            f"Distinct tasks typed Core {scope}. {TASK_TYPE_DEF}",
        ),
        (
            f"{prefix}supplemental_task_count",
            f"Distinct tasks typed Supplemental {scope}.",
        ),
        (
            f"{prefix}unclassified_task_count",
            f"Distinct tasks with blank Task Type {scope}.",
        ),
        (
...
MAPPING_DATE_COLS = [
    (
        "first_mapping_update",
        "Earliest Tasks-to-DWAs mapping date among contributing rows (YYYY-MM).",
    ),
    (
        "latest_mapping_update",
        "Latest Tasks-to-DWAs mapping date among contributing rows (YYYY-MM).",
    ),
    (
        "domain_sources",
        "Joined unique O*NET domain sources among contributing rows.",
    ),
]


def write_output_codebook() -> None:
    """Write the codebook for all outputs in this directory."""
    update_codebook(
        OUTPUT_DIR / "codebook.md",
        section="iwa_mapping",
        title="IWA to O*NET-SOC mapping",
        source="pipeline/build_iwa_onet_mapping.py",
        intro=(
            "IWAs are mapped to O*NET-SOC 2019 occupations through the O*NET "
            "30.2 chain IWA Reference -> DWA Reference -> Tasks to DWAs. See "
            "iwa_onet_mapping_report.md for method detail and validation."
        ),
        files=[
            {
                "name": "iwa_to_onet_soc_via_tasks.csv",
                "description": (
                    "One row per IWA-occupation pair with link counts and "
                    "candidate within-IWA allocation weights."
                ),
                "columns": IWA_ID_COLS
                + ONET_SOC_ID_COLS
                + _count_cols()
                ...
                + [
                    (
                        "mapping_method",
                        "Constant 'task_dwa_reference', naming the join path used.",
                    ),
                    ("notes", "Constant caveat string describing the mapping."),
                ],
            },
            {
                "name": "iwa_to_onet_soc_via_tasks_detail.csv",
                "description": (
                ...
                + [
                    ...
                    (
                        ...
                        "Number of survey incumbents rating the task (from "
                        "Task Statements).",
                    ),
                    ...
                    (
                        "task_statement_date",
                        "Task Statements update date (mm/YYYY).",
                    ),
                    (
                        "task_statement_domain_source",
                        "Task Statements data collection source.",
                    ),
                    (
                        "task_type_clean",
                        "task_type with blank values recoded to Unclassified.",
                    ),
                ],
            },
            {
                "name": "iwa_occupation_links.csv",
                "description": (
                    "Slim IWA-occupation edge table with per-link counts, "
                    "coverage counts, and the candidate weights."
                ),
                "columns": IWA_ID_COLS
                + ONET_SOC_ID_COLS
                + _count_cols("link_")
            ...
            {
                "name": "iwa_occupation_counts.csv",
                "description": "Coverage summary, one row per IWA.",
                "columns": IWA_ID_COLS
                + [
                    (
                        "occupation_count",
            ...
            },
            {
                ...
                "description": "Coverage summary, one row per occupation.",
                "columns": ONET_SOC_ID_COLS
                + [
                    (
                        "iwa_count",
                        "Number of distinct IWAs linked to the occupation.",
            ...
            {
                "name": "iwa_weight_correlations.csv",
                "description": (
                    "Pairwise correlations between candidate weight columns "
                    "across IWA-occupation links."
                ),
                "columns": [
    ...
    print(f"updated codebook -> {OUTPUT_DIR / 'codebook.md'}")
