...
df_eloundou["oews_emp_was_imputed"] = missing_emp_with_exposure
log.info(
...
df_2010["oews_emp_was_imputed"] = df_2010["oews_tot_emp_allocated"].isna()
log.info(
...
df_aei_occ["oews_emp_was_imputed"] = df_aei_occ["oews_tot_emp_allocated"].isna()
...
# ======================================================================================
# Step 9: Update the output codebook
# ======================================================================================
GROUP_ID_DEF = (
...
SOC_2010_ID_COLS = [
    (
        "soc_2010",
        "Six-digit SOC 2010 occupation code (8-digit O*NET-SOC 2010 codes "
        "truncated to the parent SOC code).",
    ),
...
OEWS_SOC_2010_COLS = [
    (
        "oews_tot_emp_allocated",
        "OEWS SOC 2018 employment (after the broad-code adjustment) allocated "
        "to SOC 2010 by splitting each SOC 2018 code's employment equally "
        "across its SOC 2010 crosswalk partners, then summing by SOC 2010. NA "
        "when no crosswalk partner has OEWS employment.",
    ),
    (
        "oews_tot_emp_imputed",
        "oews_tot_emp_allocated with missing values filled with the median "
        "allocated employment across SOC 2010 occupations. Used as the "
        "employment weight and the per-capita denominator.",
    ),
    (
        "oews_emp_was_imputed",
        "True when oews_tot_emp_allocated was missing and oews_tot_emp_imputed "
        "holds the median-employment fill value.",
    ),
    (
        "oews_a_mean",
        "OEWS annual mean wage, averaged across the occupation's SOC 2018 "
        "crosswalk partners weighted by allocated employment.",
    ),
]

MEASURE_DEFS = {
    "task_count": "AEI conversation count for the occupation's tasks",
    "automation_count": (
        "AEI conversations in collaboration modes classed as automation "
        "(directive + feedback loop)"
    ),
    "augmentation_count": (
        "AEI conversations in collaboration modes classed as augmentation "
        "(validation + task iteration + learning)"
    ),
}
PLATFORM_DEFS = {
    "1p_api": "first-party API traffic",
    "claude_ai": "Claude.ai traffic",
    "total": "both platforms combined",
}
WEIGHT_DEFS = {
    "equal": (
        "each task's counts split equally across the occupations sharing the "
        "task (weight 1/n occupations)"
    ),
    "emp": (
        "each task's counts split by employment: the occupation's "
        "oews_tot_emp_imputed share among occupations sharing the task (equal "
        "split when the task's total employment is not positive)"
    ),
}

aei_count_cols = []
for weight, weight_def in WEIGHT_DEFS.items():
    for platform, platform_def in PLATFORM_DEFS.items():
        for measure, measure_def in MEASURE_DEFS.items():
            aei_count_cols.append(
                (
                    f"{weight}_{platform}_{measure}",
                    f"{measure_def}; {platform_def}; {weight_def}. Counts are "
                    "summed across all cleaned AEI releases; tasks unobserved "
                    "in AEI contribute zero.",
                )
            )
for platform, platform_def in PLATFORM_DEFS.items():
    for measure, measure_def in MEASURE_DEFS.items():
        aei_count_cols.append(
            (
                f"emp_{platform}_{measure}_pc",
                f"emp_{platform}_{measure} divided by oews_tot_emp_imputed "
                "(zero employment recoded to NA). Scale is per worker; note "
                "that the OpenAI wage analysis reports usage per million "
                "workers instead.",
            )
        )
...
update_codebook(
    OUT / "codebook.md",
    section="occupational_characteristics",
    title="Occupational characteristics panels",
    source="pipeline/occupational_characteristics.py",
    intro=(
        "The default run uses May 2022 OEWS. Runs with `--oews-year`/"
        "`--output-tag` write the same columns to tagged file names (e.g., "
        "`occupations_aei_oews_2024.csv`)."
    ),
    files=[
        {
            "name": "occupations_eloundou_et_al.csv",
            "description": (
                "SOC 2018 panel with Eloundou et al. (2023, 'GPTs are GPTs') "
                "occupation-level exposure scores and OEWS employment and "
                "wage fields."
            ),
            "columns": [
                ...
                (
                    "oews_occ_title",
                    "OEWS national occupation title for the matched code.",
                ),
                (
                    "oews_tot_emp",
                ...
                (
                    ...
                    "Employment after the broad-code fallback: the exact-match "
                    "value, or the broad code's employment split equally "
                    "across its detailed child codes without their own OEWS "
                    "row.",
                ),
                (
                    "oews_broad_match",
                    "True when employment came from the broad-code fallback "
                    "rather than an exact SOC 2018 match.",
                ),
                (
                    "oews_tot_emp_imputed",
                    "oews_tot_emp_adjusted with missing employment filled "
                    "with the median across occupations, only for occupations "
                    "that have exposure ratings.",
                ),
                (
                    "oews_emp_was_imputed",
                    "True when the occupation has exposure ratings but no "
                    "OEWS employment, so oews_tot_emp_imputed holds the "
                    "median-employment fill value.",
                ),
            ],
        },
        {
            "name": "occupations_aei.csv",
            "description": (
                "SOC 2010 panel of AEI task-level usage apportioned to "
                "occupations via O*NET task statements, with OEWS employment "
                "allocated across the SOC 2010<->2018 crosswalk. Count "
                "columns follow the pattern `{weight}_{platform}_{measure}`."
            ),
            "columns": SOC_2010_ID_COLS + OEWS_SOC_2010_COLS + aei_count_cols,
        },
        {
            "name": "occupations_aei_auto_aug_2025_03_27.csv",
            "description": (
                "SOC 2010 panel built from the AEI occupation-level "
                "automation/augmentation release (2025-03-27), collapsed from "
                "8-digit O*NET-SOC codes."
            ),
            "columns": SOC_2010_ID_COLS
            + OEWS_SOC_2010_COLS
            + [
                (
                    "pct_occ_scaled",
                    "AEI occupation-level usage share (percent of usage), "
                    "summed across 8-digit O*NET-SOC sub-occupations. "
                    "Occupations with O*NET tasks that are absent from the "
                    "release are recoded to 0; occupations with no O*NET "
                    "tasks stay NA.",
                ),
                (
                    "augmentation_weighted_ratio",
                    "AEI augmentation share of classified usage, averaged "
                    "across sub-occupations weighted by pct_occ_scaled "
                    "(unweighted mean when the weights sum to zero); same "
                    "zero-recode rule as pct_occ_scaled.",
                ),
                (
                    "automation_weighted_ratio",
                    "AEI automation share of classified usage; same "
                    "aggregation and zero-recode rule as "
                    "augmentation_weighted_ratio.",
                ),
                (
                    "pct_occ_scaled_pc",
                    "pct_occ_scaled divided by oews_tot_emp_imputed (zero "
                    "employment recoded to NA). Scale is share per worker.",
                ),
            ],
        },
    ],
