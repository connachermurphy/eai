...
def write_codebook(release_dir: Path) -> None:
    """Write the codebook for one release's cleaned files."""
    collab_list = ", ".join(COLLAB_TYPES)
    update_codebook(
        release_dir / "codebook.md",
        section="aei_cleaned",
        title="Cleaned AEI task-level usage",
        source="pipeline/clean_aei.py",
        intro=(
            "One file per platform. Raw AEI rows are filtered to the GLOBAL "
            "geography and pivoted wide, one row per O*NET task."
        ),
        files=[
            {
                "name": "aei_cleaned_claude_ai.csv / aei_cleaned_1p_api.csv",
                "description": (
                    "Task-level Claude usage for the release, for Claude.ai "
                    "and first-party API traffic respectively."
                ),
                "columns": [
                    ("task", "O*NET task statement text."),
                    (
                        "task_count",
                        "Total conversations classified to the task (from the "
                        "onet_task facet). NA when the task appears in the "
                        "collaboration facet only.",
                    ),
                    (
                        "task_pct",
                    ...
                    (
                        "{collab}_count",
                        "Conversations for the task in the given collaboration "
                        f"mode, for collab in: {collab_list}. Missing modes "
                        "are filled with 0.",
                    ),
                    (
                        "{collab}_pct",
                        "Percent share of the task's conversations in the "
                        "given collaboration mode (0-100).",
                    ),
                ],
            }
        ],
    )
    logger.info("[%s] Updated codebook: %s", release_dir.name, release_dir)
